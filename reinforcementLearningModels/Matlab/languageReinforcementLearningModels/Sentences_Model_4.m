...
function R = Sentences_Model_4(x,p_Accuracy,p_ReactionTime,p_RewardPositivity,step)
try
    PE = []; RPE = []; %Clear variables
    learning_rate = x(1); %Learning rate determined by fmincon function
    temperature = x(2); %Temperature determined by fmincon function
    weights = x(3); %Initial weights determined by fmincon function
